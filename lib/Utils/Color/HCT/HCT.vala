namespace He.Color {
  ...
  public string hex_from_hct (He.Color.HCTColor hct, double contrast) {
    // If color is mono…
    if (hct.c < 1.0001 || contrast < 0.0001 || contrast > 99.9999) {
      return hexcode_argb (He.MathUtils.argb_from_lstar (contrast));
      // Else…
    } else {
      hct.h = He.MathUtils.sanitize_degrees (hct.h);
      double hr = hct.h / 180 * Math.PI;
      double y = He.MathUtils.y_from_lstar (contrast);
      int exact_answer = find_result_by_j (hr, hct.c, y);
