...
#define RESOURCE_PATH "/com/fyralabs/Helium/Screenshot/"
...
static GOptionEntry entries[] = {
    {"image", 'i', 0, G_OPTION_ARG_STRING, &option_image,
     "Generate only one image", "NAME"},
...
typedef struct {
  ...
  HeStyleManager *style_manager;
} ScreenshotData;

static void screenshot_data_free(ScreenshotData *data) {
...
static GdkPixbuf *crop_alpha(GdkPixbuf *pixbuf) {
  ...
  return gdk_pixbuf_new_subpixbuf(pixbuf, left, top, right - left,
                                  bottom - top);
}

static gboolean draw_paintable_cb(ScreenshotData *data) {
  ...
  if (GTK_IS_NATIVE(data->widget)) {
    ...
    gtk_native_get_surface_transform(GTK_NATIVE(data->widget), &transform_x,
                                     &transform_y);
    x = floor(transform_x);
    y = floor(transform_y);
  ...
  gdk_paintable_snapshot(data->paintable, snapshot, widget_width,
                         widget_height);
  ...
  texture = gsk_renderer_render_texture(
      renderer, node, &GRAPHENE_RECT_INIT(0, 0, width, height));
...
static void draw_paintable(ScreenshotData *data) {
...
static GtkCssProvider *load_css(const char *name) {
...
static gboolean take_screenshot_cb(ScreenshotData *data) {
  ...
  if (data->hscroll_widget) {
    ...
    adj = gtk_scrolled_window_get_hadjustment(
        GTK_SCROLLED_WINDOW(data->hscroll_widget));
  ...
  if (data->vscroll_widget) {
    ...
    adj = gtk_scrolled_window_get_vadjustment(
        GTK_SCROLLED_WINDOW(data->vscroll_widget));
...
static void take_screenshot(const char *name, gboolean dark, GFile *input_dir,
                            GFile *output_dir) {
  char *input_name;
  GFile *input_file;
  char *input_path;
  ...
  data->style_manager = he_style_manager_new();

  if (dark) {
    data->style_manager->is_dark = TRUE;
  ...
  he_style_manager_register(data->style_manager);
...
static inline char *get_shortname(const char *basename) {
...
static void init_libhelium(void) {
  he_init();
  ...
  g_object_set(gtk_settings_get_default(), "gtk-enable-animations", FALSE,
               "gtk-font-name", "Manrope 11", "gtk-icon-theme-name", "Hydrogen",
               "gtk-decoration-layout", ":close", "gtk-hint-font-metrics", TRUE,
               NULL);
}

static GList *list_images(GFile *input_dir) {
  ...
  enumerator =
      g_file_enumerate_children(input_dir, G_FILE_ATTRIBUTE_STANDARD_NAME,
...
static void process_image(const char *name, GFile *input_dir,
                          GFile *output_dir) {
...
static void run_screenshot(GFile *input_dir, GFile *output_dir) {
  GList *children = NULL;
  GList *l;
...
static void run_list_images(GFile *input_dir) {
